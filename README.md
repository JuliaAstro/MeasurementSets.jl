...
`LIKE` / regex, `BETWEEN`, a function library, 1-based array indexing /
slicing (`DATA[1,1]`, `UVW[3]`, `V[1:4,1]`), `ORDER BY`, column
projection → a `RefTable`), `groupby` (GROUP BY + `g*` aggregates +
HAVING, string or
