...
`UVW[-1]`, `V[end-2:end,1]`), `BETWEEN` / `NOT BETWEEN`, and bitwise
`& | ^ ~` (`^` = xor) *are* supported, but no boolean-mask subscripts,
no units or date/time / measures functions, no `GROUP BY ROLLUP`, no
