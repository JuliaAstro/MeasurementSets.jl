...
`UVW[-1]`, `V[end-2:end,1]`) and `BETWEEN` / `NOT BETWEEN` *are*
supported, but no bitwise operators, no boolean-mask subscripts, no
units or date/time / measures functions, no `GROUP BY ROLLUP`, no
general M:N cross-product join, no `UPDATE` array-slice assignment.
